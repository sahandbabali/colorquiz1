...
var size = Object.keys(colordata).length;
console.log(size);

var arraydata = Object.keys(colordata).map(function (key) {
...
document.getElementById("startgame").addEventListener("click", function (e) {
  document.getElementById("changingviews").innerHTML = "";
  setgame();
});
...
function setgame() {
  ...
  var currentchoise = Math.floor(Math.random() * arraydata.length);
  console.log(currentchoise);

  currentcorrect = Math.floor(Math.random() * 4) + 1;
  ...
  for (let i = 1; i < 5; i++) {
    if (i == currentcorrect) {
      ...
      document.getElementById(i).innerText =
        arraydata[Math.floor(Math.random() * arraydata.length)].name;
    }
  }
}
...
document
  ...
  .addEventListener("click", function (e) {
    if (e.target.id == currentcorrect) {
      ...
      e.target.id == 2 ||
      e.target.id == 3 ||
      (e.target.id == 4 && e.target.id != currentcorrect)
    ) {
